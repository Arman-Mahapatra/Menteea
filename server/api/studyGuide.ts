...
/**
 * Handles incoming Study Guide generation requests.
 */
export async function handleStudyGuide(req: Request, res: Response) {
  try {
    ...
    if (!doc && Array.isArray(selectedDocuments)) {
      ...
      if (clientDoc) {
        logger.info(`[handleStudyGuide] Synchronizing client document to single source of truth: ID=${clientDoc.id}`);
        doc = {
          id: clientDoc.id,
          name: clientDoc.name,
    ...
    const result = await chatService.generateStudyGuide(
      activeKey,
      doc.name,
      documentContext
    );
...
export default handleStudyGuide;