...
/**
 * Handles incoming Mind Map generation requests.
 */
export async function handleMindMap(req: Request, res: Response) {
  try {
    ...
    // Retrieve selected document from centralized DocumentService (the single source of truth)
    ...
    // (e.g. following a server restart), we gracefully register it from the client payload
    if (!doc && Array.isArray(selectedDocuments)) {
      const clientDoc = selectedDocuments.find((d: any) => d.id === documentId);
      if (clientDoc) {
        logger.info(`[handleMindMap] Synchronizing client document to single source of truth: ID=${clientDoc.id}`);
        doc = {
          id: clientDoc.id,
          name: clientDoc.name,
    ...
    const result = await chatService.generateMindMap(
      activeKey,
      doc.name,
      documentContext
    );
